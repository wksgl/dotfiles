...
SWWW_CMD="awww img --transition-type $ANIM_TYPE --transition-duration $ANIM_DURATION"
...
fetch_current_wall() {
    local raw_line
    read -r raw_line < <(awww query 2>/dev/null)
...
cleanup() {
    ...
    if is_blur_filename "$cname" && [[ -f "$LAST_CLEAR_FILE" ]]; then
        ...
        [[ -f "$original" ]] && awww img "$original" --transition-type none
    fi
    exit 0
}
# 捕获退出信号
